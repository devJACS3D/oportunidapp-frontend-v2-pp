...
@Component({
  selector: 'app-user-interviewes',
  templateUrl: './user-interviewes.component.html',
  styleUrls: ['./user-interviewes.component.scss']
})
export class UserInterviewesComponent implements OnInit {
	...
	private async loadEntidades() {
		try {
			let resp = await this.api.get('user/interviews', null, this._currentPage, this._ItemsPerPage).toPromise() as ApiResponse;
	...
	public delete(Entity: any) {
		this._confirmMessage = "¿Desea eliminar el proceso para la vacante " + Entity.vacancy.name + "?";
	...
	public async confirm($event: Event) {
		if ($event) {
			...
			try {
				let resp = await this.api.delete(Entities.userVacancies, this._EntityToDelete.id).toPromise() as ApiResponse;
	...
	async getInterview() {
		let citiesResp = await this.api.get('user/interviews', null, 1, 1000).toPromise();
