...
export class SuccessStoriesComponent implements OnInit {
	...
	private async loadEntidades(params?: Object){
		try {
			let resp = await this.api.get(Entities.successStories, null, this._currentPage, this._ItemsPerPage, params).toPromise() as ApiResponse;
	...
	public viewDetail(item: any){
		// this.router.navigate([`./user-view/${item.userId}`], { relativeTo: this.activatedRoute }); 
		this.router.navigate(['../../../success-story-detail/', item.id], { relativeTo: this.activatedRoute }); 
	}
	...
	public delete(Entity: any) {
		this._confirmMessage = "¿Desea eliminar el caso de éxito " + Entity.name + "?";
	...
	public async confirm($event: Event){
		if($event){
			...
			try {
				let resp = await this.api.delete(Entities.successStories, this._EntityToDelete.id).toPromise() as ApiResponse;
